...
/**
 * @author Mazoh 我的订单管理 --订单完成
 */
@SuppressLint("NewApi")
public class MyOrderComplete extends BaseFragment implements IAppointOperatable, IMyOrderOperater, ItemForNoneOrderLayoutOperater {
...
//                intent.putExtra("order", order);
	...
	private void loadOrders(int start, int end) {
		...
		actionTypes.add(ChargeOrder.ACTION_PAIED);
		actionTypes.add(ChargeOrder.ACTION_COMMANDED);
		if (user_id != null && !"".equals(user_id)) {
			WebAPIManager.getInstance().getMyOrder(user_id, null, actionTypes, orders.size(), MyConstant.PAGE_SIZE, start, end, new WebResponseHandler<List<ChargeOrder>>(getActivity()) {
						@Override
						public void onStart() {
							super.onStart();
						...
						public void onSuccess(WebResponse<List<ChargeOrder>> response) {
							...
							if (orders == null || orders.size() < MyConstant.PAGE_SIZE) {
							...
							if (!EmptyUtil.isEmpty(orders)) {
								...
								if (myOrdersAdapter.getCount() <= 0) {
									...
									yuyue_charge_layout.setVisibility(View.VISIBLE);
									btn_yuyue_charge.setOnClickListener(new OnClickListener() {
