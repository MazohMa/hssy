...
/**
 * Created by Joke on 2015/8/25.
 */
public class DurationDialog extends Dialog implements View.OnClickListener, AdapterView
		.OnItemClickListener {
	...
	public DurationDialog(Context context, int theme) {
		super(context, theme);
		init(context);
	}

	protected DurationDialog(Context context, boolean cancelable, OnCancelListener
			cancelListener) {
		...
		protected ViewHolder newHolder() {
			return new ViewHolder() {
				...
				protected void update() {
					...
					if (hour < 1) {
						tv.setText((int) (hour * 60) + "分钟");
					} else {
						tv.setText((int) hour + "小时");
					}
